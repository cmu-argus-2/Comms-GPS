...
gps = gps_driver.GPS(uart, debug=False)  # Use UART/pyserial
...
while True:
    ...
    if current - last_print >= 1.0:
        ...
        if not updated:
            ...
            if nav_data["fix_mode"] < 2:
                print("No fix")
                continue
            else:
                print("Fix: {0}".format(nav_data["fix_mode"]))
                print("Latitude: {0}".format(nav_data["latitude"]))
                print("Longitude: {0}".format(nav_data["longitude"]))
                print("Satellites: {0}".format(nav_data["number_of_sv"]))
